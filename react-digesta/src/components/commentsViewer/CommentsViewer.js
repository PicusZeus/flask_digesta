...
const CommentsViewer = ({paragraphus_id, repliedId, paragraphus}) => {
    ...
    const updateCommentedParagrahiOnDelete = (paragraphus_id) => {
        ...
        if (comments.filter(com => com.user_id === user_id).length === 1) {
            const rest = commentedParagraphi.filter(paragraphus => paragraphus.id !== paragraphus_id)
    ...
    const deleteMutation = useMutation({
        ...
        onSuccess: () => {
            ...
            queryClient.invalidateQueries(["comment", "paragraphus", username, paragraphus_id])
    ...
    const {data: comments} = useQuery({
        ...
        onError: (e) => {
            if (e.response.status === 401) {
                ...
                notificationSetter.setNotificationError("Błąd serwera", "Nie udało się załadować komentarzy")
            }
        }
    })
