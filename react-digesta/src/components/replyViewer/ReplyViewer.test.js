...
describe("ReplyViewer logged out", () => {
  ...
  test("doesnt open delete modal when delete button is clicked and when logout", async () => {
    ...
    const deleteModal = await screen.queryByRole("button", {
      name: /potwierdzam/i,
    });
  ...
  test("doesnt set liked when logged out", () => {
    ...
    expect(
      screen.queryByRole("button", { name: /heart_check/i })
...
describe("ReplyViewer logged in", () => {
  ...
  test("opens delete modal when delete button is clicked and when logout", async () => {
    ...
    const deleteModal = await screen.findByRole("button", {
      name: /potwierdzam/i,
    });
    expect(deleteModal).toBeInTheDocument();
    // await act(()=>{
    await act(async () => {
      await user.click(deleteModal);
    });
  ...
  test("doesnt set liked when logged out", async () => {
    ...
    const likedComment = await screen.findByRole("button", {
      name: /heart_check/i,
    });
