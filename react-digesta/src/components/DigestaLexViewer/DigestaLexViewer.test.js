...
jest.mock("../DigestaToc/DigestaTocMobile/DigestaTocMobileParagraphi/DigestaTocMobileParagraphi",()=> jest.fn())

jest.mock("../DigestaParagraphusViewer/DigestaParagraphusViewer", ()=>jest.fn())
describe('DigestaLexViewer', () => {
    ...
    test('renders the lex data when fetched successfully', async () => {
        ...
        const latAddress = await screen.findByText("ULPIANUS libro primo institutionum")
        expect(latAddress).toBeInTheDocument()

        const arrowBack = await screen.findByRole('link', {name: /arrow_back_ios/i})
        expect(arrowBack).toHaveAttribute('href', '/digesta/1')

        const arrowForward = await screen.findByRole('link', {name: /arrow_forward_ios/i})
        ...
        const linkOpus = await screen.findByRole('link', {name: /Liber 1 Libri Institutionum/i})
