...
describe('DigestaTocDesktopLex', () => {
  ...
  test('renders the component with lex and paragraphi items', () => {
    render(
      <MemoryRouter>
        <DigestaTocDesktopLex address="D.1" lex={lex} legesLength={1} path="/" />
      </MemoryRouter>
    );
  ...
  render(
    ...
    await act(async ()=>{await user.click(leks)})

    expect(mockDispatch).toHaveBeenCalledWith(digestaActions.setChosenLexId(lex.id));
  });
