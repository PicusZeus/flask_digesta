...
const CommentViewer = ({paragraphus_id, comment, username, onDelete, queryClient}) => {
    ...
    const saveEditedMutation = useMutation({
        ...
        onMutate: () => {
            notificationSetter.setNotificationPending("Rozpoczęto", "Aktualizacja w toku")
        },
        onSuccess: () => {
            notificationSetter.setNotificationSuccess("Sukces", "Wpis został zaktualizowany")
        },
        onError: () => {
            notificationSetter.setNotificationError("Niepowodzenie", "Problem z siecią lub autoryzacją")
        }
    })
    ...
    const likeCommentMutation = useMutation({
        ...
        onError: () => {
            notificationSetter.setNotificationError("Błąd Sieci", "Nie udało się połączyć z serwerem")
        }
    })
        ...
        <li className={classes.comment_item}>
            ...
            <div className={classes.comment_container}>
                ...
                <div className={classes.comment_item__container}>
                    ...
                    <div className={classes.comment_item__container}>
                        ...
                        <div className={classes.comment_item__actions}>
                            ...
                            <button className="material-symbols-outlined"
                                    onClick={() => saveEditedMutation.mutate({
                                        newText: commentText.current.value,
                                        id: comment.id
                                    })}
                                    disabled={!editOn}>
                                save
                                <span className={classes.tooltiptext}>zapisz zmiany</span></button>
                   ...
                   <RepliesViewer repliedId={comment.id} queryClient={queryClient}
