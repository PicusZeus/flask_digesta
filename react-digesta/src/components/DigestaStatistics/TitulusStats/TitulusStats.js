...
const TitulusStats = () => {
  ...
  const { data: stats, isFetching } = useQuery(
    getTitulusStatsQuery(params.titulus_id)
  );

  if (isFetching) {
    return <Spinner />;
  }
  const authorsMoreFivePercent = stats.jurists_authorship.filter(
    (a) => a.authorship > 5
  );
  const authorsLessFivePercent = stats.jurists_authorship.filter(
    (a) => a.authorship <= 5
  );

  const authorsSets = [authorsMoreFivePercent, authorsLessFivePercent];

  const operaMoreFivePercent = stats.opera_coverage.filter(
    (o) => o.coverage > 5
  );
  const operaLessFivePercent = stats.opera_coverage.filter(
    (o) => o.coverage <= 5
  );
  ...
  return (
    <>
      <h1 className={classes.titulus_stats__title}>
        Księga {stats.titulus.book.book_nr}
      </h1>
      <h2 className={classes.titulus_stats__subtitle}>
        tytuł {stats.titulus.number}{" "}
      </h2>
      <h2 className={classes.titulus_stats__subtitle}>
        {stats.titulus.title_lat}
      </h2>
      <h3 className={classes.titulus_stats__info}>
        Wybierz jurystę, dla którego chcesz poznać listę jego prac zawartych w
        tym tytule
      </h3>
      ...
      <BookAuthorshipChart
        authors={authorsSets[authorsSetIndex]}
        book_id={stats.titulus.book.id}
        titulus_id={stats.titulus.id}
      />

      <h3 className={classes.titulus_stats__info}>
        Wybierz pracę cytowaną w tym tytule, dla której chcesz poznać jej udział
        we poszczególnych tytułach księgi
      </h3>
      <form className={classes.titulus_stats__options}>
        <label htmlFor="selectOpera">
          Zobacz prace z udziałem w tym tytule
        </label>
