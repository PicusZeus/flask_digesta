...
const JuristTitulusStats = () => {
    ...
    const {
        data: stats,
        isFetching
    } = useQuery(getJuristTitulusStatsQuery(params.jurysta_id, params.book_id, params.titulus_id))
    if (isFetching) {
        return <Spinner/>
    }

    return <>{stats && <>
            <h1 className={classes.jurist_titulus_stats__title}>{stats.author.name}</h1>
            <h3 className={classes.jurist_titulus_stats__info}>w księdze {stats.titulus.titulus.book.book_nr}</h3>
            <h3 className={classes.jurist_titulus_stats__info}>w tytule {stats.titulus.titulus.number}</h3>
            <h3 className={classes.jurist_titulus_stats__info}>{stats.titulus.titulus.title_lat}</h3>
            <h4 className={classes.jurist_titulus_stats__info}>Wybierz pracę, dla której chcesz poznać statystyki dla całych Digestów</h4>
