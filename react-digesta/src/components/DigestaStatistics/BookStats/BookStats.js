...
const BookStats = () => {
  ...
  const operaSets = [
    operaMoreOnePercent,
    operaLessOnePercentMoreOnePromile,
    operaLessOnePromile,
  ];
  return (
    <>
      <h1 className={classes.book_stats__title}>
        {stats.book.book_latin_name}
      </h1>
      <h2 className={classes.book_stats__subtitle}>
        KSIĘGA {stats.book.book_nr}
      </h2>
      <h3 className={classes.book_stats__info}>
        Tytuły i ich udział w objętości księgi {stats.book.book_nr}
      </h3>
      <h3 className={classes.book_stats__info}>
      ...
      <h3 className={classes.book_stats__info}>
        Juryści i ich udział w objętości księgi {stats.book.book_nr}
      </h3>
      <h3 className={classes.book_stats__info}>
        Wybierz jurystę, dla którego chcesz poznać dalsze statystyki w ramach
        księgi {stats.book.book_nr}
      </h3>
      ...
      {stats && (
        <BookAuthorshipChart
          authors={authorsSets[authorshipSetIndex]}
          book_id={stats.book.id}
        />
      )}

      <h3 className={classes.book_stats__info}>
        Dzieła jurystów i ich udział w objętości księgi {stats.book.book_nr}
      </h3>
      <h3 className={classes.book_stats__info}>
        Wybierz pracę, dla której chcesz poznać dalsze statystyki w ramach
        księgi {stats.book.book_nr}
      </h3>
      <form className={classes.book_stats__options}>
        ...
        <select id="selectOpera" onChange={onOptionOp}>
          <option value="0">ponad jeden procent</option>
          <option value="1">
            poniżej jednego procenta a ponad jeden promil
          </option>
