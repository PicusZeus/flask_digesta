...
const JuristStats = () => {
  ...
  const { data: stats, isFetching } = useQuery(
    getJuristStatsQuery(params.jurysta_id)
  );
  if (isFetching) {
    return <Spinner />;
  }
  ...
  return (
    <>
      {stats && (
        <h1 className={classes.jurist_stats__title}>
          {stats.jurist.name} w Digestach
        </h1>
      )}
      ...
      <h3 className={classes.jurist_stats__info}>
        Wybierz pracę, dla której chcesz poznać jej udział w poszczególnych
        księgach Digestów
      </h3>
