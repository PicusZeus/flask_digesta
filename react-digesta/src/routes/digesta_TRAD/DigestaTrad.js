...
import DigestaTocDesktopBooks from "../../components/DigestaToc/DigestaTocDesktop/DigestaTocDesktopBooks/DigestaTocDesktopBooks";
...
const DigestaTrad = () => {
  ...
  const { data: books, isFetching } = useQuery({
    ...getBooksQuery(),
    onError: () => {
      notificationsSetter.setNotificationError(
        "Błąd ładowania",
        "Nie powiodło się ładowanie spisu treści"
      );
    },
  });
  if (isFetching) {
